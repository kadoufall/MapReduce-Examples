...
public class Main {
    public static void main(String[] args) throws Exception {
        final String INPUT_PATH = "hdfs://localhost:9000/secondSort";           // 任务输入路径
        final String OUTPUT_PATH = "hdfs://localhost:9000/output_secondSort";    // 任务输出路径
        ...
        Job job = Job.getInstance(conf, "SecondarySortApp");            // 设置一个作业对象
        job.setJarByClass(sort.basic.Main.class);                                  // 设置处理该作业对象的类

        job.setMapperClass(MyMapper.class);                             // 设置处理Map步的类
        job.setMapOutputKeyClass(IntPair.class);                        // 设置Map步输出的key类型的类
        ...
        job.setGroupingComparatorClass(GroupingComparator.class);       // 设置分组函数的类
